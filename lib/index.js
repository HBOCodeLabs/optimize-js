 ...
 *
 * Modification from original: Instead of embedding the source map, return the source map object
 * so we can save it to a file later.
 *
 */
...
function optimizeJs (jsString, opts) {
  ...
  var outMap
  if (opts.sourceMapName) {
    outMap = magicString.generateMap({ file: opts.sourceMapName, source: opts.sourceFileName })
  }
  return {
    source: out,
    sourceMap: outMap
